...
int state = 1; //当state=0时关闭LED灯，state=1时开启LED灯
int brightness = 40; //存储方式数值，128为中间值
...
//RBD::Button button(LEDC, INPUT_PULLUP);
/*
void control()
{
 val = digitalRead(LEDC); //读取按钮状态并存储至val
  //检测按钮状态变化过程（设定LED灯状态）
  ...
  if ((val == LOW) && (old_val == LOW)) 
  {
    ...
    if (state == 1 && (millis() - startTime) > 2000) 
    {
      brightness++;//亮度变量逐渐+1
      delay(10);//延时10毫秒避免增加速度过快
      if (brightness > 255) 
      { //255是亮度变量最大值（PWM输出最大值）
  ...
  old_val = val; //存储当前val变量状态，用来判断按钮变化状态
    ...
    digitalWrite(PIN_LCD_BL, LOW); //关闭LED灯
}
*/
...
void sleep()
{
  ...
  {
    ledStatus = !ledStatus;
    digitalWrite(PIN_LCD_BL, ledStatus);
  }
  delay(100);
}
*/
...
//  if ((digitalRead(ENCODER_PUSH_BUTTON) == LOW)&&(digitalRead(LEDC) == LOW))
/*
...
void drawSprite()
{
  ...
  if (isMenuMode() or cmdBand) drawMenu();    
    ...
    spr.drawString("Off",48,64+(2*16),2);
    spr.setTextDatum(MC_DATUM);
  }
  ...
  if (currentMode == FM) {
    ...
    spr.drawString("FM Mono",278,31,2);
  } else {
    spr.fillSmoothRoundRect(240,20,76,22,4,TFT_WHITE);
    spr.fillSmoothRoundRect(241,21,74,20,4,TFT_BLACK);
...
void DrawBatteryLevel(int batteryLevel) {
  ...
  */
  ...
  if ( batteryLevel == 4 ) {   // To do: Animated icon to charge mode!
    chargeLevel=18;
    batteryLevelColor=TFT_GREEN;
  }
  /*
...
void batteryMonitor(bool forced) {
  if(battery.getBatteryVolts() >= MIN_USB_VOLTAGE){
    if (!batteryCharging or forced) {
      ...
      }else if(batteryLevel < 50 && batteryLevel >= 5 ){
        currentBatteryLevel = 2;
      //}else if(batteryLevel < 35 && batteryLevel >= 10 ){
      //  currentBatteryLevel = 2;
